...
function drawObstacles(ctx) {
    state.mapObstacles.forEach(obs => {
        ...
        if (obs.type === 'tree') {
            ...
            ctx.beginPath();
            ctx.arc(obs.x, obs.y, obs.radius * 1.1, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#16a34a';
            ctx.beginPath();
            ctx.arc(obs.x, obs.y, obs.radius * 0.9, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#22c55e';
            ctx.beginPath();
            ctx.arc(obs.x - obs.radius * 0.2, obs.y - obs.radius * 0.2, obs.radius * 0.5, 0, Math.PI * 2);
            ctx.fill();
        } else if (obs.type === 'rock') {
            ctx.fillStyle = '#44403c';
            ctx.beginPath();
            ctx.ellipse(obs.x, obs.y, obs.radius * 1.2, obs.radius * 0.85, 0.4, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#57534e';
            ctx.beginPath();
            ctx.ellipse(obs.x - obs.radius * 0.1, obs.y - obs.radius * 0.15, obs.radius * 0.9, obs.radius * 0.65, 0.4, 0, Math.PI * 2);
            ctx.fill();
        } else {
            // crate
            ...
            ctx.closePath();
            ctx.fill();
            ctx.strokeStyle = `hsl(30, 60%, ${25 + hpRatio * 15}%)`;
            ctx.lineWidth   = 2;
            ctx.stroke();
        }
        ctx.restore();
...
function drawLoot(ctx) {
    ...
    state.itemsOnGround.forEach(item => {
        ...
        ctx.save();
        ctx.shadowColor = item.type === 'medkit' ? '#22c55e' : '#fbbf24';
        ctx.shadowBlur  = 12;
...
export function drawCharacter(ctx, player, isLocal) {
    ...
    // Tělo
    ctx.shadowColor = isLocal ? color : 'rgba(0,0,0,0.5)';
